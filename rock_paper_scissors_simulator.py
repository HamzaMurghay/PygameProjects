# This is a program that spawns 33 rock, papers and scissors respectively and allows a full on battle to see which can win and take the lead, this program was inspired by watching a short, link: https://www.youtube.com/watch?v=vPGI4UkNcKw, the program works by making the rock paper and scissor objects respectively go after their defeatable enemy (example: rock is strong against scissors, same with scissors towards paper etc.) and also avoid their undefeatable enemy (paper in rock's case, rock in scissors case) when they get closer than a certain distance
...
clock = pg.time.Clock()
text_font = pg.font.Font('venv/fonts/primeform_pro/PrimeformProDemo-Light.otf', 20)
text_smallerfont = pg.font.Font('venv/fonts/primeform_pro/PrimeformProDemo-Light.otf', 13)


class Rock:
    rock_surface = pg.image.load('venv/graphics/rockT.png').convert_alpha()
    speed_x = speed_y = 1
...
def create_all_objects():
    ...
    rock_list = [Rock() for i in range(33)]

    paper_list = [Paper() for i in range(33)]

    scissor_list = [Scissor() for i in range(33)]
...
def place_all_objects():
    ...
    for rock in rock_list:
        rock.rectangle = rock.rock_surface.get_rect(topleft = (random.randint(4, 577), random.randint(82, 575)))  # Creating rock rectangle object
...
current_play_state_surface = playb_surface = pg.image.load('venv/graphics/playbutton.jpg').convert_alpha()  # Might give explanation about these two line, I don't know yet
...
def chase_closest_defeatable_enemy(chaser, chased_list, chased_num):
    if chased_num > 0:
        ...
        closest_chased_entity = chased_entities_of_particular_distance[chaser_and_chased_distances.index(min(chaser_and_chased_distances))]
...
def avoid_closest_undefeatable_enemy(avoider, obj_avoided_list, obj_avoided_num):
    if obj_avoided_num > 0:
        ...
        closest_obj_to_avoid_entity = objs_avoided_of_particular_distance[avoider_obj_avoided_distances.index(min(avoider_obj_avoided_distances))]
...
while True:
    ...
    for event in pg.event.get():
        ...
        if (event.type == pg.MOUSEBUTTONUP and current_play_state_rectangle.collidepoint(mouse.get_pos())) or (event.type == pg.KEYDOWN and event.key == pg.K_SPACE):
        ...
        if (event.type == pg.MOUSEBUTTONUP and reset_button_rectangle.collidepoint(mouse.get_pos())) or (event.type == pg.KEYDOWN and event.key == pg.K_r):
            if winner is None: pass
            else: winner.clear()
    ...
    game_border = pg.draw.rect(screen, 'Black',  pg.Rect(0, 80, 600, 520), 2)  # Draws black border which represents maximum limit for object travel
    ...
    for scissor in scissor_list:
        ...
        for paper in paper_list:                                                        # from here
            if scissor.rectangle.colliderect(paper.rectangle):
                ...
                scissor_list.append(paper)                                             # to here is code in order to change the class of a paper when it touches a scissor, similar code is there in every loop for each object when it touches its opponent that it can defeat
                # pg.time.delay(2)
...
# rgb values for green : 41, 240, 97     rgb values for gray: 220 220, 220


# screen.blit(rock1.rock_surface, mouse.get_pos())
#py game.draw.rect(screen, 'pink',(15, 100, mouse.get_pos()[0]-15, mouse.get_pos()[1]-100)
